...
@section('content')
    ...
    <div id="toast-notification" class="fixed top-4 right-4 z-50 animate-slide-in-right max-w-md">
        <div class="bg-gradient-to-r @if(session('success')) from-green-600 to-green-700 @elseif(session('error')) from-red-600 to-red-700 @else from-blue-600 to-blue-700 @endif text-white px-6 py-4 rounded-xl shadow-2xl">
            <div class="flex items-center gap-3">
                ...
                @endif
                <div class="flex-1">
                    <p class="font-bold text-sm">{{ session('success') ?? session('error') ?? session('info') }}</p>
                </div>
                <button onclick="document.getElementById('toast-notification').remove()" class="text-white/80 hover:text-white transition">
            ...
            <!-- Progress Bar -->
            <div class="h-1 bg-white/20 rounded-full mt-3 overflow-hidden">
                <div id="toast-progress" class="h-full bg-white/80 rounded-full transition-all" style="width: 100%; animation: shrink-width 20s linear;"></div>
            </div>
        </div>
    </div>
    <script>
            ...
            <div class="lg:col-span-3">
                ...
                @else
                    <div class="text-center py-16 bg-white/5 border-2 border-dashed border-white/10 rounded-2xl">
                        ...
                        @auth
                            <form id="request-episode-form" action="{{ route('request.store') }}" method="POST" class="mt-6 inline-flex">
                                @csrf
                                ...
                                <button type="submit" id="request-episode-btn" class="px-5 py-3 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-bold rounded-xl transition-all shadow-lg shadow-red-600/30 flex items-center gap-2">
                                    <span id="request-btn-text">Request Tambah Episode</span>
                                    <svg id="request-btn-spinner" class="hidden animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            ...
                            <div id="request-toast" class="hidden fixed top-4 right-4 z-50 max-w-md animate-slide-in-right">
                                <div id="request-toast-content" class="text-white px-6 py-4 rounded-xl shadow-2xl">
                                    <div class="flex items-center gap-3">
                                        <div id="request-toast-icon"></div>
                                        <div class="flex-1">
                                            <p id="request-toast-message" class="font-bold text-sm"></p>
                                        </div>
                                        <button onclick="document.getElementById('request-toast').classList.add('hidden')" class="text-white/80 hover:text-white transition">
                                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
                                            </svg>
                                        </button>
                            ...
                            <script>
                                document.getElementById('request-episode-form').addEventListener('submit', function(e) {
                                    e.preventDefault();
                                    
                                    const form = this;
                                    const btn = document.getElementById('request-episode-btn');
                                    ...
                                    const toastContent = document.getElementById('request-toast-content');
                                    const toastIcon = document.getElementById('request-toast-icon');
                                    const toastMessage = document.getElementById('request-toast-message');
                                    ...
                                    .then(data => {
                                        ...
                                        toastContent.className = 'bg-gradient-to-r from-green-600 to-green-700 text-white px-6 py-4 rounded-xl shadow-2xl';
                                        toastIcon.innerHTML = '<svg class="w-6 h-6 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>';
                                        toastMessage.textContent = data.message || 'Request berhasil dikirim! Admin akan meninjau.';
                                        ...
                                        btnText.textContent = '✓ Request Terkirim';
                                    ...
                                    .catch(error => {
                                        // Show error toast
                                        toast.classList.remove('hidden');
                                        toastContent.className = 'bg-gradient-to-r from-red-600 to-red-700 text-white px-6 py-4 rounded-xl shadow-2xl';
                                        toastIcon.innerHTML = '<svg class="w-6 h-6 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/></svg>';
                                        toastMessage.textContent = 'Terjadi kesalahan. Silakan coba lagi.';
                                        
                                        // Reset button
                                        btn.disabled = false;
                                        btnText.textContent = 'Request Tambah Episode';
                                        spinner.classList.add('hidden');
