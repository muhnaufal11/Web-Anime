    ...
    <div class="max-w-7xl mx-auto px-4 py-8 sm:py-16">
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 sm:gap-8">
            ...
            <div class="lg:col-span-3">
                ...
                <!-- Episodes Grid -->
                <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-6" id="latestEpisodesGrid" data-api-url="{{ route('episodes.latest') }}">
                    @php $order = 0; @endphp
                    @foreach($latestEpisodes as $anime)
                        @foreach($anime->episodes as $episode)
                        @php $shouldBlur = $anime->shouldBlurPoster(); @endphp
                        <!-- Debug Order: {{ ++$order }}. {{ $anime->title }} -->
                        <a href="{{ $shouldBlur ? '#' : route('watch', $episode) }}" class="group block episode-card" data-episode-id="{{ $episode->id }}" @if($shouldBlur) onclick="event.preventDefault(); alert('Konten 18+ - Anda harus login dan berusia minimal 18 tahun untuk mengakses.')" @endif>
